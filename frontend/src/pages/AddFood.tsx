...
export default function AddFood() {
  ...
  const handleAddFood = (food: any) => {
    const id = Math.random().toString(36).substr(2, 9); // We need the ID for undo
          ...
          {search && (
            ...
            >
              <div className="flex justify-between items-center px-2">
                <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-500">Results for "{search}"</h3>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={() => {
              ...
              {filteredMock.length > 0 ? (
                ...
                <div className="text-center py-16 bg-white/5 border border-dashed border-white/10 rounded-[40px]">
                  <p className="text-zinc-500 text-lg mb-6">We couldn't find "{search}" in our database.</p>
                  <Button 
                    onClick={() => {
                      setCustomFood({ ...customFood, name: search });
                      setShowCustomForm(true);
...
function FoodCard({ food, onAdd }: { food: any, onAdd: () => void }) {
  return (
    <Card className="bg-white/5 border-white/10 p-4 rounded-[24px] group hover:bg-white/10 transition-all duration-300">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${food.isVeg ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {food.isVeg ? (
                <span className="text-[8px] font-black px-1.5 py-0.5 bg-emerald-500/20 text-emerald-500 rounded-md border border-emerald-500/30 uppercase tracking-tighter">Veg</span>
              ) : (
                <span className="text-[8px] font-black px-1.5 py-0.5 bg-red-500/20 text-red-500 rounded-md border border-red-500/30 uppercase tracking-tighter">Non-Veg</span>
              )}
            </div>
